...
15b. Every unanswered card carries **Answer in words**. Its `comment` verdict requires text, writes `💬 Answer in words:`, collapses the options, moves the card to "Waiting on agent", and counts as answered. A reviewer's plain reply on an unanswered card counts the same way. A card the agent addressed or withdrew leaves "Needs my review"; a reviewer reply reopens it.
