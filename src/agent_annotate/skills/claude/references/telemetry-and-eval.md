...
| `round_submitted` | `POST /api/rounds/submit` | `comment_ids`, `verdict_counts {accept, reject, comment}`, `undecided_ids`, `note`, `by`, `session_id` |
...
`round_discarded`, `session_push`. `session_push` is excluded because it
...
1. **Per-slug shape.** Comments, decision requests, the accept/reject/comment/
   undecided split, median and p90 time to verdict, agent against reviewer
   event counts, 15-minute authoring bursts, pushes by delivery, versions.
...
3. **Verdict to agent reaction, and unanswered requests.** Median and p90
   ...
   bucketed by age, and the fifteen oldest with their prompts.
4. **`comment` verdict taxonomy.** Every free-text verdict classified, counted
   both across all free text and across verdict text alone, with each item
   listed so the classification is auditable.
5. **Decision-prompt quality against outcome.** Answered, comment and accept
...
Any one of these is a regression, not a data point:
...
The last three are correctness bugs and have no acceptable rate. Read them off
sections 7 (bystander, loose), 1 (delivery) and the publish events. The first
two come from sections 3 and 2: an unanswered card usually means the card did
